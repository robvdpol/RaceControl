namespace RaceControl.Services.F1TV;

public static class Constants
{
    public const string LoginUrl = @"https://account.formula1.com/#/en/login";
    public const string ApiEndpointUrl = @"https://f1tv.formula1.com";
    public const string AuthenticateUrl = @"https://api.formula1.com/v2/account/subscriber/authenticate/by-password";
    public const string ImageUrl = @"https://f1tv.formula1.com/image-resizer/image";
    public const string ApiKey = @"fCUCjWrKPu9ylJwRAv8BpGLEgiAuThx7";
}